...
Swayamvar
A ceremony where a Bride chooses her Groom from an array of eligible bachelors is called Swayamvar. But this is a Swayamvar with difference.

An array of Bride-to-be will choose from an array of Groom-to-be. The arrangement at this Swayamvar is as follows
...
Third line contains a string in lowercase, of length N containing initial state of grooms-to-be.
Each string contains only lowercase r and m stating person at that index drinks rum (for r) or mojito (for m).
Output
...
brides_freq["r"] = brides.count('r')
brides_freq["m"] = len(brides) - brides_freq["r"]

grooms_freq["r"] = grooms.count("r")
grooms_freq["m"] = len(grooms) - grooms_freq["r"]

ans = len(brides)

while ans > 0:
    curr = brides[0]
    if grooms_freq[curr] <= 0:
        break
    brides.pop(0)
    grooms.remove(curr)
    grooms_freq[curr]-=1
