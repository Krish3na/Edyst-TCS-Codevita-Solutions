...
for i in range(t):
    ...
    repeated_freq=[1]*len(arr)
    
    for i in range(1,n):
        curr_ind=i
        if not visited[curr_ind]:
            next_ind=arr[curr_ind]
            visited[curr_ind]=True
            while next_ind!=curr_ind:
                if not visited[next_ind]:
                    visited[next_ind]=True
                    next_ind=arr[next_ind]
                    repeated_freq[curr_ind]+=1
            
        running_lcm=lcm(running_lcm,repeated_freq[curr_ind])
    print(running_lcm)
